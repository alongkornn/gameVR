...
namespace VRKeys
{
	...
	public class Keyboard : MonoBehaviour
	{
		...
		/// Initialization.
		/// </summary>
		IEnumerator Start()  // แก้ไขจาก 'private IEnumerator Start()'
		{
			...
			if (inputSubsystems.Count > 0)
			{
				...
				// ใช้ TrackingOriginModeFlags แทน TrackingOriginMode
		...
		private void Update()
		{
			...
			{
				leftHand.transform.localPosition = leftHandDevice.TryGetFeatureValue(CommonUsages.devicePosition, out Vector3 leftPosition) ? leftPosition : leftHand.transform.localPosition;
				leftHand.transform.localRotation = leftHandDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out Quaternion leftRotation) ? leftRotation : leftHand.transform.localRotation;
			}

			if (rightHandDevice.isValid)
			{
				rightHand.transform.localPosition = rightHandDevice.TryGetFeatureValue(CommonUsages.devicePosition, out Vector3 rightPosition) ? rightPosition : rightHand.transform.localPosition;
				rightHand.transform.localRotation = rightHandDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out Quaternion rightRotation) ? rightRotation : rightHand.transform.localRotation;
			}
		}
		...
		IEnumerator EnableWhenInitialized() // แก้ไขจาก 'public IEnumerator EnableWhenInitialized()'
		{
